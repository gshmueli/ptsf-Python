...
f_naive_seas = NaiveForecaster(strategy='last', sp=12).fit(ridership_train)
fh = ForecastingHorizon(ridership_test.index, is_relative=False)
pred_naive = f_naive.predict(fh=fh)
pred_seas = f_naive_seas.predict(fh=fh)

style.use('ggplot')
figure, axs = plt.subplots(2, 1, sharex=False, figsize=(8, 7))

# Plot 1: actuals and forecasts
axs[0] = plot_series(ridership, pred_naive, pred_seas, ax=axs[0])
ptsf_theme(axs[0], colors=['black','red','green'], idx=[0,1,2], lty=['-', '--', '--'],
         labels=['Actual', 'Naive', 'Seasonal Naive'], do_legend=True, legend_loc='below')
axs[0] = ptsf_train_test(axs[0], ridership_train.index, ridership_test.index)

# Plot 2: errors
naive_err_train = ridership_train['Ridership'] - ridership_train['Ridership'].shift(1)
snaive_err_train = ridership_train['Ridership'] - ridership_train['Ridership'].shift(12)
naive_err_test = ridership_test['Ridership'] - pred_naive.iloc[:,0]
snaive_err_test = ridership_test['Ridership'] - pred_seas.iloc[:,0]

axs[1] = plot_series(naive_err_train, snaive_err_train, naive_err_test, snaive_err_test, ax=axs[1],
                    title='Errors', y_label='Error')
ptsf_theme(axs[1], colors=['red','green']*2, idx=[0,1,2,3], lty=['-', '-','--','--'])
axs[1] = ptsf_train_test(axs[1], ridership_train.index, ridership_test.index)
plt.subplots_adjust(hspace=0.5)

plt.savefig('Ch03_Fig_3_7_Amtrak.pdf', format='pdf', bbox_inches='tight')
plt.show()
