...
pred_naive = f_naive.predict(fh=ridership_test.index)
...
ptsf_theme(axs[0], colors=['black','blue','red'], idx=[0,1,2], lty=['-', '--', '--'],
         labels=['Actual', 'Fixed Origin Forecasts', 'Rolling Origin Forecasts'],
         do_legend=True, legend_loc='below')
...
axs[1] = plot_series(naive_roll_fwd_err_train, naive_err_test,
                     naive_roll_fwd_err_test, ax=axs[1], title='Errors', y_label='Error')
ptsf_theme(axs[1], colors=['red','blue','red'], idx=[0,1,2], lty=['-','--','--'],
           labels=['Rolling Origin Errors (Train)', 'Fixed Origin Errors (Test)',
                   'Rolling Origin Errors (Test)'], do_legend=True, legend_loc='below')
